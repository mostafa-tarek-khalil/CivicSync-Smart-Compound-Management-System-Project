...
const createTicket = async (req, res) => {
  try {
    ...
    res.status(error.statusCode || 500).json({
      message: error.message || "Failed to create maintenance ticket",
    });
  }
};
...
const getResidentTickets = async (req, res) => {
  try {
    const tickets = await ticketService.getResidentTickets(req.user.userId);
    res.status(200).json({
...
const getTicketDetails = async (req, res) => {
  try {
    const ticket = await ticketService.getTicketDetails(req.params.id, req.user.userId);
