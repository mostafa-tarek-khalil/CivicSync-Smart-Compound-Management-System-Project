...
const createOffer = async (req, res) => {
  try {
    const offer = await offerService.createOffer(req.params.ticketId, req.user.userId, req.body);
    res.status(201).json({
...
const updateOffer = async (req, res) => {
  try {
    const offer = await offerService.updateOffer(req.params.id, req.user.userId, req.body);
    res.status(200).json({
...
const withdrawOffer = async (req, res) => {
  try {
    const offer = await offerService.withdrawOffer(req.params.id, req.user.userId);
    res.status(200).json({
